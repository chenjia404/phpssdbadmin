		...
		<ul class="nav navbar-nav">
			...
			<li style="line-height: 40px;">
				...
				<?php foreach((array)$confs as $k=>$conf){ ?>
					<option value="<?php echo "{$conf['host']}:{$conf['port']}";?>"<?php echo "{$conf['host']}:{$conf['port']}"==$conf_k? ' selected="selected"' : ''; ?>><?php if(isset($conf["name"])) echo "{$conf["name"]}({$conf['host']}:{$conf['port']})";else echo $k; ?></option>
				<?php } ?>
				</select>
			</li>
		</ul>
